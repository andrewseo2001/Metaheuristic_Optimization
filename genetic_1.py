...
class Optimizer:
    ...
    def mutate(self, individual):
        mutated = [coord.copy() for coord in individual]
        for waypoint in mutated:
            for i in range(len(waypoint)):
                if random.random() < self.mutation_rate:
                    low, high = self.coord_bounds[list(self.coord_bounds.keys())[i]]
                    waypoint[i] = np.random.uniform(low, high)
        return mutated
...
x_pos_min, x_pos_max = 0.65,0.75
y_pos_min, y_pos_max = -0.25,0.25
z_pos_min, z_pos_max = 0.24,0.6
...
initial_guess = [[0.7783378978151467, 0.0796063648497023, 0.2653798174242051, 79.24151937223138, -6.499334816987137, 106.60931907334992], [0.6713507327507355, 0.3410063432040249, 0.4627946336915488, 100.61212759649523, -39.312127850235726, 109.41304266467888], [0.62905251766778, -0.1768639523589407, 0.6257823544943764, 101.07013849306203, -36.95297161162059, 95.5838412953172], [0.641966847416767, 0.23893183429378406, 0.608885911816746, 79.24512046514164, -35.939116757600296, 84.47325535281094]]
